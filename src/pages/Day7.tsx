...
const Day7: React.FC = () => {
  ...
  const initializeGrid = useCallback(() => {
    console.log('Initializing Day 7 grid...');
    const newCells: Cell[] = [];
    const gridSize = 7;
    ...
    // SCONE (down): col 2, rows 0-4
    for (let row = 0; row <= 4; row++) {
      activeCells.add(`${row}-2`);
    }
    
    // SUNRISE (across): row 3, cols 0-6
    for (let col = 0; col <= 6; col++) {
      activeCells.add(`3-${col}`);
    }
    
    // SHOWER (across): row 6, cols 0-5
    for (let col = 0; col <= 5; col++) {
      activeCells.add(`6-${col}`);
    }
    
    // SUDS (down): col 0, rows 3-6
    for (let row = 3; row <= 6; row++) {
      activeCells.add(`${row}-0`);
    }
    
    // STIR (down): col 5, rows 3-6
    for (let row = 3; row <= 6; row++) {
    ...
    morningPuzzle.clues.forEach((clue) => {
  ...
  const isValidAnswer = useCallback((cell: Cell, value: string) => {
    const cluesForCell = morningPuzzle.clues.filter(clue => {
  ...
  const handleCellSelect = useCallback((cellId: string) => {
    ...
    const [row, col] = cellId.split('-').map(Number);
    const matches = morningPuzzle.clues.filter(clue => {
      if (clue.direction === 'across') {
  ...
  const handleStart = useCallback(() => {
    ...
    setTimeElapsed(0);
    setCurrentClue(morningPuzzle.clues[0]);
    const firstClue = morningPuzzle.clues[0];
  ...
  useEffect(() => {
    // Force scroll to Day 7 button (rightmost position) for mobile
    const scrollToDay7 = () => {
      if (scrollAreaRef.current) {
        ...
        viewports.forEach(viewport => {
          if (viewport && viewport.scrollTo) {
            // Scroll to maximum right position for Day 7
    ...
    const timers = [
      setTimeout(scrollToDay7, 0),
      setTimeout(scrollToDay7, 100),
      setTimeout(scrollToDay7, 300),
      setTimeout(scrollToDay7, 500),
      setTimeout(scrollToDay7, 1000),
      setTimeout(scrollToDay7, 2000)
    ];
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted flex flex-col justify-center items-center px-4">
        <div className="text-center animate-fade-in">
          ...
          <p className="text-xl text-muted-foreground animate-fade-in">Morning Routine Edition - Day 7</p>
          <p className="text-sm text-muted-foreground mt-2 animate-fade-in">Loading your morning puzzle...</p>
        </div>
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-background via-background to-muted overflow-hidden">
      ...
      {/* Timer in top right - smaller for Day 7 */}
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="hidden lg:block fixed right-4 top-1/2 transform -translate-y-1/2 w-80">
          <CluesPanel clues={morningPuzzle.clues} />
        </div>
      </div>
      ...
      <div className="fixed bottom-16 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-sm px-4">
        <ScrollArea ref={scrollAreaRef} className="w-full whitespace-nowrap rounded-md bg-background/80 backdrop-blur-sm border p-1">
          <div className="flex gap-2">
            ...
            <Button variant="secondary" size="sm" disabled className="shrink-0">Day 7</Button>
          </div>
